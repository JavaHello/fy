...
trait Fy {
    type Resp;
    fn fy(&mut self, content: &str) -> Self::Resp;
}

struct YouDaoFy {
    qry_data: HashMap<String, String>,
}

impl YouDaoFy {
    fn new() -> Self {
        let mut fy = YouDaoFy {
            qry_data: HashMap::new(),
        };
        fy.qry_data.insert("from".to_owned(), "auto".to_owned());
        fy.qry_data.insert("to".to_owned(), "auto".to_owned());
        fy.qry_data.insert("appKey".to_owned(), APP_KEY.to_owned());
        fy.qry_data.insert("signType".to_owned(), "v3".to_owned());
        fy
    }
}

impl Fy for YouDaoFy {
    type Resp = FyResp;
...
impl YouDaoFy {
    ...
    fn gen_qry(&mut self, q: &str) -> String {
        let now = SystemTime::now();
        self.qry_data.insert("q".to_owned(), q.to_owned());

        self.qry_data.insert(
            "salt".to_owned(),
        ...
        m.input_str(Self::sign_str(&self.qry_data).as_str());
        self.qry_data.insert("sign".to_owned(), m.result_str());
    ...
    fn sign_str(data: &HashMap<String, String>) -> String {
        let mut result = String::new();
        result.push_str(data.get("appKey").unwrap());
        let sign_str = YouDaoFy::input(data.get("q").unwrap());
    ...
    fn query_url(&self) -> String {
        let mut result = self.query_str();
        result.insert_str(0, YOUDAO_URL);
        result
    }

    fn query_str(&self) -> String {
        let mut result = String::new();
        for item in self.qry_data.iter() {
...
mod tests {
    ...
    fn test_sign_str() {
        ...
        let res = YouDaoFy::sign_str(&mut map);
        let mut t = String::from("abcd");
        t.push_str(APP_SECRET);
        assert_eq!(t, res);
        map.insert(
            "q".to_owned(),
            "123456789axxx一二三四五六七八九十".to_owned(),
        );
